...
if ($query->rowCount() > 0) {
    while ($rows = $query->fetch()) {
        $mql = "SELECT * FROM res_category WHERE c_id=:c_id";
        $categoryQuery = $db->query($mql, ['c_id' => $rows['c_id']]);
        $row = $categoryQuery->fetch();
        ...
        echo '<td>' . $rows['url'] . '</td>';
        echo '<td>' . $rows['o_hr'] . '</td>';
        echo '<td>' . $rows['c_hr'] . '</td>';
        echo '<td>' . $rows['o_days'] . '</td>';
        echo '<td>' . $rows['address'] . '</td>';
        echo '<td><div class="col-md-3 col-lg-8 m-b-10">';
        echo '<center><img src="Res_img/' . $rows['image'] . '" class="img-responsive radius" style="min-width:150px;min-height:100px;"/></center>';
        echo '</div></td>';
        echo '<td>' . $rows['date'] . '</td>';
        echo '<td><a href="delete_restaurant.php?res_del=' . $rows['rs_id'] . '" onclick="return confirm(\'Are you sure?\');" class="btn btn-danger btn-flat btn-addon btn-xs m-b-10"><i class="fa fa-trash-o" style="font-size:16px"></i></a>';
        echo '<a href="update_restaurant.php?res_upd=' . $rows['rs_id'] . '" class="btn btn-info btn-flat btn-addon btn-sm m-b-10 m-l-5"><i class="fa fa-edit"></i></a></td>';
        echo '</tr>';
    }
} else {
