    ...
    $db = new Database($config['database']);
        ...
        if (strlen($_SESSION['user_id']) == 0) {
            ...
            if (isset($_POST['update'])) {
                ...
                $status = $_POST['status'];
                $remark = $_POST['remark'];
                $query = $db->query("INSERT INTO remark(frm_id, status, remark) VALUES (?, ?, ?)", [$form_id, $status, $remark]);
                $sql = $db->query("UPDATE users_orders SET status = ? WHERE o_id = ?", [$status, $form_id]);
                    ...
                    <body>
                        ...
                        <div style="margin-left:50px;">
                            <form name="updateticket" id="updatecomplaint" method="post">
                                ...
                                <?php
                        $ret1 = $db->query("SELECT * FROM users_orders WHERE o_id = ?", [$_GET['newform_id']]);
                        $ro = $ret1->fetch();
                        $ret2 = $db->query("SELECT * FROM users WHERE u_id = ?", [$ro['u_id']]);
                        while ($row = $ret2->fetch()) {
                        ?>
